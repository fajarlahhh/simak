			...
			<li class="nav-profile">
				<a href="javascript:;" data-toggle="nav-profile">
					...
					<div class="info">
						...
						{{ $nama_pegawai }}
						<small>{{ Auth::user()->pengguna_nip }}</small>
					</div>
				</a>
			</li>
			<li>
			...
			@php
				...
				function renderSubMenu($value, $currentUrl) {
					...
					foreach ($value as $key => $menu) {
						if(Auth::user()->can(substr($menu['url'], 1)) || Auth::user()->getRoleNames()[0] == 'admin'){
							...
							$subMenu .= '
								<li class="'. $hasSub .' '. $active .'">
									<a href="'. (Auth::user()->can(strtolower($hasTitle)) || Auth::user()->role('admin')? $menu['url']: '#') .'">'. $hasCaret . $hasTitle . $hasHighlight .'</a>
									'. $subSubMenu .'
								</li>
							';
						}
				...
				foreach (config('sidebar.menu') as $key => $menu) {
					if(Auth::user()->can(strtolower($menu['title'])) || Auth::user()->getRoleNames()[0] == 'admin' || $menu['title'] == 'Dashboard'){
...
<script>
	function help() {		
		swal("Bantuan", "IT PDAM Giri Menang", "info");
	}
</script>
@endpush
