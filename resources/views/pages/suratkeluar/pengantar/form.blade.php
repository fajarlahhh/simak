...
@section('title', ' | '.$aksi.' Surat Pengantar')
...
@section('header')
	<h1 class="page-header">Surat Pengantar <small>{{ $aksi }} Data</small></h1>
@endsection

@section('subcontent')
<form action="{{ route('pengantar.'.strtolower($aksi)) }}" name="form-wizard" method="post" data-parsley-validate="true" data-parsley-errors-messages-disabled="" enctype="multipart/form-data">
    @csrf
	...
	<div class="panel panel-inverse" data-sortable-id="form-stuff-1">
		...
		<div class="panel-body p-5">
            @if ($aksi == 'Edit')
            <input type="hidden" name="pengantar_nomor" value="{{ $aksi == 'Edit'? $data->pengantar_nomor: old('pengantar_nomor') }}" readonly/>
            @if ($catatan)
            ...
            <div class="tab-content">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" readonly class="form-control datepicker" name="pengantar_tanggal" required value="{{ date('d F Y', strtotime($aksi == 'Edit'? $data->pengantar_tanggal:(old('pengantar_tanggal')? old('pengantar_tanggal'): now()))) }}" data-parsley-group="step-1"/>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Sifat</label>
                                    <input class="form-control"  type="text" name="pengantar_sifat" value="{{ $aksi == 'Edit'? $data->pengantar_sifat: old('pengantar_sifat') }}" />
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Lampiran</label>
                                    <input class="form-control"  type="text" name="pengantar_lampiran" value="{{ $aksi == 'Edit'? $data->pengantar_lampiran: old('pengantar_lampiran') }}" />
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Perihal</label>
                                    <textarea class="form-control" rows="3" name="pengantar_perihal">{{ $aksi == 'Edit'? $data->pengantar_perihal: old('pengantar_perihal') }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-8">
                ...
                <div class="tab-pane fade" id="default-tab-2">
                    <fieldset>
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="pengantar_kepada_awal" value="{{ $kepada? $kepada[0]: (old('pengantar_kepada_awal')? old('pengantar_kepada_awal'): "Kepada Yth :") }}" />
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="pengantar_kepada_akhir" value="{{ $kepada? $kepada[2]: (old('pengantar_kepada_akhir')? old('pengantar_kepada_akhir'): "di Tempat") }}" />
                        </div>
                    </fieldset>
                </div>
                @endif
                <div class="tab-pane fade {{ $edit == 2? "active show": "" }}" id="default-tab-3">
                    <fieldset>
                        <textarea class="form-control" rows="3" id="editor2"  name="pengantar_isi">{{ $aksi == 'Edit'? $data->pengantar_isi: old('pengantar_isi') }}</textarea>
                    </fieldset>
                </div>
                @if ($edit == 1)
                <div class="tab-pane fade" id="default-tab-4">
                    <fieldset>
                        <div class="form-group input-group-sm">
                            ...
                            <select class="form-control selectpicker" name="pengantar_pejabat" data-live-search="true" data-style="btn-info" data-width="100%">
                        ...
                        <div class="form-group input-group-sm">
                            ...
                            <select class="form-control selectpicker" name="pengantar_jenis_ttd" data-live-search="true" data-style="btn-info" data-width="100%">
                                <option value="1" {{ ($aksi == 'Edit' && $data->pengantar_ttd == 1? 'selected': (old('pengantar_jenis_ttd') == 1? 'selected': '')) }}>QR Code</option>
                                <option value="2" {{ ($aksi == 'Edit' && $data->pengantar_ttd != 1? 'selected': (old('pengantar_jenis_ttd') == 1? 'selected': '')) }}>Gambar</option>
                            </select>
                        </div>
                        <div class="note note-default">
        ...
        function hapus(id, ket) {
            Swal.fire({
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        url: '{{ url("/pengantar/hapus/lampiran") }}?file=' + id,
                        type: "POST",
                        data: {
                            "_method": 'DELETE'
                        },
